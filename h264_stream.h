...
typedef struct {
  ...
  int run_length_minus1[8]; // up to num_slice_groups_minus1, which is <= 7 in
                            // Baseline and Extended, 0 otheriwse
  int top_left[8];
  int bottom_right[8];
...
/**
   H264 stream
   Contains data structures for all NAL types that can be handled by this
   library. When reading, data is read into those, and when writing it is
   written from those. The reason why they are all contained in one place is
   that some of them depend on others, we need to have all of them available to
...
#define NAL_UNIT_TYPE_CODED_SLICE_DATA_PARTITION_A                             \
  2 // Coded slice data partition A
#define NAL_UNIT_TYPE_CODED_SLICE_DATA_PARTITION_B                             \
  3 // Coded slice data partition B
#define NAL_UNIT_TYPE_CODED_SLICE_DATA_PARTITION_C                             \
...
#define NAL_UNIT_TYPE_DPS 16             // Depth Parameter Set
                                         // 17..18    // Reserved
#define NAL_UNIT_TYPE_CODED_SLICE_AUX                                          \
  19 // Coded slice of an auxiliary coded picture without partitioning
#define NAL_UNIT_TYPE_CODED_SLICE_SVC_EXTENSION                                \
  20 // Coded slice of SVC extension
     // 20..23    // Reserved
     // 24..31    // Unspecified
...
// as per footnote to Table 7-6, the *_ONLY slice types indicate that all other
// slices in that picture are of the same type
...
// 7.4.3.1 Table 7-7 reordering_of_pic_nums_idc operations for reordering of
...
// 7.4.3.3 Table 7-9 Memory management control operation
