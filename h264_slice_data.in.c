    ...
    if (!mb->transform_size_8x8_flag || !h->pps->entropy_coding_mode_flag) {
      for (int i4x4 = 0; i4x4 < 4; i4x4++) // each 4x4 sub-block of block
      {
        if (CodedBlockPatternLuma & (1 << i8x8)) {
          if (MbPartPredMode(mb->mb_type, 0) == Intra_16x16) {
            structure(residual_block)(b, mb->Intra16x16ACLevel[i8x8 * 4 + i4x4],
                                      15);
          } else {
...
void structure(residual_block_cavlc)(bs_t *b, int *coeffLevel,
                                     int maxNumCoeff) {
  int level[256];
  int run[256];
...
void structure(residual_block_cabac)(bs_t *b, int *coeffLevel,
                                     int maxNumCoeff) {
  if (maxNumCoeff == 64) {
    coded_block_flag = 1;
  } else {
