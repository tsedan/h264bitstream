 ...
 *
 * Written by Alex Izvorski <aizvorski@gmail.com> and Alex Giladi
...
#define printf(...)                                                            \
  fprintf((h264_dbgfile == NULL ? stdout : h264_dbgfile), __VA_ARGS__)
...
int read_nal_unit(h264_stream_t *h, uint8_t *buf, int size) {
  ...
  if (0) {
    rbsp_size =
        size * 3 / 4; // NOTE this may have to be slightly smaller (3/4 smaller,
                      // worst case) in order to be guaranteed to fit
  }
  ...
  if (nal->nal_unit_type == 14 || nal->nal_unit_type == 21 ||
  ...
  case NAL_UNIT_TYPE_SUBSET_SPS:
    ...
    if (1) {
      memcpy(h->sps_subset_table[h->sps_subset->sps->seq_parameter_set_id],
             h->sps_subset, sizeof(sps_subset_t));
      // memcpy(h->sps_subset_table[h->sps_subset->sps->seq_parameter_set_id]->sps,
      // h->sps_subset->sps, sizeof(sps_t));
      // memcpy(h->sps_subset_table[h->sps_subset->sps->seq_parameter_set_id]->sps_svc_ext,
      // h->sps_subset->sps_svc_ext, sizeof(sps_svc_ext_t));
      // h->sps_subset_table[h->sps_subset->sps->seq_parameter_set_id]->additional_extension2_flag
...
void read_prefix_nal_unit_svc(nal_t *nal, bs_t *b) {
  if (nal->nal_ref_idc != 0) {
    ...
    if ((nal->nal_svc_ext->use_ref_base_pic_flag ||
...
void read_scaling_list(bs_t *b, int *scalingList, int sizeOfScalingList,
                       int *useDefaultScalingMatrixFlag) {
  // NOTE need to be able to set useDefaultScalingMatrixFlag when reading, hence
  // passing as pointer
  int lastScale = 8;
  int nextScale = 8;
  int delta_scale;
...
void read_subset_seq_parameter_set_rbsp(sps_subset_t *sps_subset, bs_t *b) {
  ...
  case 83:
  case 86:
    read_seq_parameter_set_svc_extension(sps_subset,
                                         b); /* specified in Annex G */
    ...
    if (sps_svc_ext->svc_vui_parameters_present_flag) {
      read_svc_vui_parameters_extension(sps_svc_ext,
                                        b); /* specified in Annex G */
    }
    break;
  default:
...
void read_seq_parameter_set_svc_extension(sps_subset_t *sps_subset, bs_t *b) {
  ...
  if (sps_subset->sps->chroma_format_idc == 1 ||
      sps_subset->sps->chroma_format_idc == 2) {
...
void read_pic_parameter_set_rbsp(h264_stream_t *h, bs_t *b) {
  ...
  if (pps->num_slice_groups_minus1 > 0) {
    ...
    if (pps->slice_group_map_type == 0) {
      for (int i_group = 0; i_group <= pps->num_slice_groups_minus1;
           i_group++) {
...
void read_slice_layer_rbsp(h264_stream_t *h, bs_t *b) {
  ...
  if (slice_data != NULL) {
    ...
    uint8_t *sptr = b->p + (!!b->bits_left); // CABAC-specific: skip alignment
    ...
    // ugly hack: since next NALU starts at byte border, we are going to be
    // padded by trailing_bits;
    return;
  }
...
void read_slice_header(h264_stream_t *h, bs_t *b) {
  ...
  if (sps->pic_order_cnt_type == 0) {
    sh->pic_order_cnt_lsb =
        bs_read_u(b, sps->log2_max_pic_order_cnt_lsb_minus4 + 4); // was u(v)
  ...
  if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_P) ||
    ...
    if (sh->num_ref_idx_active_override_flag) {
      sh->num_ref_idx_l0_active_minus1 =
          bs_read_ue(b); // FIXME does this modify the pps?
      if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_B)) {
  ...
  if (pps->num_slice_groups_minus1 > 0 && pps->slice_group_map_type >= 3 &&
      pps->slice_group_map_type <= 5) {
    int v = intlog2(pps->pic_size_in_map_units_minus1 +
...
void read_ref_pic_list_reordering(h264_stream_t *h, bs_t *b) {
  ...
  if (!is_slice_type(sh->slice_type, SH_SLICE_TYPE_I) &&
    ...
    if (sh->rplr.ref_pic_list_reordering_flag_l0) {
      ...
      } while (sh->rplr.reorder_l0.reordering_of_pic_nums_idc[n] != 3 &&
               !bs_eof(b));
    }
  }
  if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_B)) {
    ...
    if (sh->rplr.ref_pic_list_reordering_flag_l1) {
      ...
      } while (sh->rplr.reorder_l1.reordering_of_pic_nums_idc[n] != 3 &&
               !bs_eof(b));
    }
  }
}
...
void read_dec_ref_pic_marking(h264_stream_t *h, bs_t *b) {
  ...
  if (h->nal->nal_unit_type == 5) {
    ...
    if (sh->drpm.adaptive_ref_pic_marking_mode_flag) {
      ...
      } while (sh->drpm.memory_management_control_operation[n] != 0 &&
               !bs_eof(b));
    }
  }
}
...
void read_slice_header_in_scalable_extension(h264_stream_t *h, bs_t *b) {
  ...
  memcpy(h->pps, h->pps_table[sh->pic_parameter_set_id], sizeof(pps_t));
  memcpy(sps_subset, h->sps_subset_table[pps->seq_parameter_set_id],
         sizeof(sps_subset_t));
  // memcpy(h->sps_subset->sps,
  // h->sps_subset_table[pps->seq_parameter_set_id]->sps, sizeof(sps_t));
  // memcpy(h->sps_subset->sps_svc_ext,
  // h->sps_subset_table[pps->seq_parameter_set_id]->sps_svc_ext,
  ...
  sh->frame_num =
      bs_read_u(b, sps_subset->sps->log2_max_frame_num_minus4 + 4); // was u(v)
  ...
  if (sps_subset->sps->pic_order_cnt_type == 0) {
    sh->pic_order_cnt_lsb = bs_read_u(
        b, sps_subset->sps->log2_max_pic_order_cnt_lsb_minus4 + 4); // was u(v)
    if (pps->pic_order_present_flag && !sh->field_pic_flag) {
      sh->delta_pic_order_cnt_bottom = bs_read_se(b);
    }
  }
  if (sps_subset->sps->pic_order_cnt_type == 1 &&
      !sps_subset->sps->delta_pic_order_always_zero_flag) {
    sh->delta_pic_order_cnt[0] = bs_read_se(b);
    if (pps->pic_order_present_flag && !sh->field_pic_flag) {
      sh->delta_pic_order_cnt[1] = bs_read_se(b);
    }
  ...
  if (nal->nal_svc_ext->quality_id == 0) {
    ...
    read_ref_pic_list_reordering(h, b);
    if ((pps->weighted_pred_flag &&
         is_slice_type(sh->slice_type, SH_SLICE_TYPE_EP)) ||
        (pps->weighted_bipred_idc == 1 &&
         is_slice_type(sh->slice_type, SH_SLICE_TYPE_EB))) {
      // svc specific
  ...
  if (pps->entropy_coding_mode_flag &&
      !is_slice_type(sh->slice_type, SH_SLICE_TYPE_EI)) {
    sh->cabac_init_idc = bs_read_ue(b);
  }
  sh->slice_qp_delta = bs_read_se(b);
  if (pps->deblocking_filter_control_present_flag) {
  ...
  // svc specific
  if (!nal->nal_svc_ext->no_inter_layer_pred_flag &&
      nal->nal_svc_ext->quality_id == 0) {
    sh_svc_ext->ref_layer_dq_id = bs_read_ue(b);
    if (sps_subset->sps_svc_ext
...
void read_dec_ref_base_pic_marking(nal_t *nal, bs_t *b) {
  ...
  if (nal->prefix_nal_svc->adaptive_ref_base_pic_marking_mode_flag) {
    ...
    } while (nal->prefix_nal_svc->memory_management_base_control_operation !=
             0);
  }
}
...
void write_prefix_nal_unit_svc(nal_t *nal, bs_t *b) {
  if (nal->nal_ref_idc != 0) {
    ...
    if (nal->prefix_nal_svc->additional_prefix_nal_unit_extension_flag) {
      while (more_rbsp_data(b)) {
        bs_write_u1(b, nal->prefix_nal_svc
...
void write_subset_seq_parameter_set_rbsp(sps_subset_t *sps_subset, bs_t *b) {
  ...
  case 83:
  case 86:
    write_seq_parameter_set_svc_extension(sps_subset,
                                          b); /* specified in Annex G */
    ...
    if (sps_svc_ext->svc_vui_parameters_present_flag) {
      write_svc_vui_parameters_extension(sps_svc_ext,
                                         b); /* specified in Annex G */
    }
    break;
  default:
...
void write_slice_header(h264_stream_t *h, bs_t *b) {
  ...
  if (sps->pic_order_cnt_type == 0) {
    bs_write_u(b, sps->log2_max_pic_order_cnt_lsb_minus4 + 4,
...
void write_slice_header_in_scalable_extension(h264_stream_t *h, bs_t *b) {
  ...
  bs_write_u(b, sps_subset->sps->log2_max_frame_num_minus4 + 4,
  ...
  if (sps_subset->sps->pic_order_cnt_type == 0) {
    bs_write_u(b, sps_subset->sps->log2_max_pic_order_cnt_lsb_minus4 + 4,
               sh->pic_order_cnt_lsb); // was u(v)
    if (pps->pic_order_present_flag && !sh->field_pic_flag) {
      bs_write_se(b, sh->delta_pic_order_cnt_bottom);
    }
  ...
  if (nal->nal_svc_ext->quality_id == 0) {
    ...
    if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_EP) ||
      ...
      bs_write_u1(b, sh->num_ref_idx_active_override_flag);
      if (sh->num_ref_idx_active_override_flag) {
        bs_write_ue(b, sh->num_ref_idx_l0_active_minus1); // FIXME does this
                                                          // modify the pps?
        if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_EB)) {
          bs_write_ue(b, sh->num_ref_idx_l1_active_minus1);
        }
...
void write_dec_ref_base_pic_marking(nal_t *nal, bs_t *b) {
  ...
  if (nal->prefix_nal_svc->adaptive_ref_base_pic_marking_mode_flag) {
    do {
      bs_write_ue(
          b, nal->prefix_nal_svc->memory_management_base_control_operation);
...
void read_debug_nal_unit_header_svc_extension(nal_svc_ext_t *nal_svc_ext,
                                              bs_t *b);
...
void read_debug_scaling_list(bs_t *b, int *scalingList, int sizeOfScalingList,
                             int *useDefaultScalingMatrixFlag);
void read_debug_subset_seq_parameter_set_rbsp(sps_subset_t *sps_subset,
                                              bs_t *b);
void read_debug_seq_parameter_set_svc_extension(sps_subset_t *sps_subset,
                                                bs_t *b);
void read_debug_svc_vui_parameters_extension(sps_svc_ext_t *sps_svc_ext,
                                             bs_t *b);
  ...
  case NAL_UNIT_TYPE_SUBSET_SPS:
    ...
    if (1) {
      ...
      memcpy(h->sps_subset_table[h->sps_subset->sps->seq_parameter_set_id]
                 ->sps_svc_ext,
             h->sps_subset->sps_svc_ext, sizeof(sps_svc_ext_t));
      h->sps_subset_table[h->sps_subset->sps->seq_parameter_set_id]
...
void read_debug_prefix_nal_unit_svc(nal_t *nal, bs_t *b) {
  if (nal->nal_ref_idc != 0) {
    ...
    nal->prefix_nal_svc->additional_prefix_nal_unit_extension_flag =
        bs_read_u1(b);
    printf(
        "nal->prefix_nal_svc->additional_prefix_nal_unit_extension_flag: %d \n",
        nal->prefix_nal_svc->additional_prefix_nal_unit_extension_flag);
    if (nal->prefix_nal_svc->additional_prefix_nal_unit_extension_flag) {
      while (more_rbsp_data(b)) {
        printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
        nal->prefix_nal_svc->additional_prefix_nal_unit_extension_data_flag =
            bs_read_u1(b);
        printf("nal->prefix_nal_svc->additional_prefix_nal_unit_extension_data_"
               "flag: %d \n",
               nal->prefix_nal_svc
                   ->additional_prefix_nal_unit_extension_data_flag);
      }
...
void read_debug_subset_seq_parameter_set_rbsp(sps_subset_t *sps_subset,
  ...
  case 83:
  case 86:
    read_debug_seq_parameter_set_svc_extension(sps_subset,
                                               b); /* specified in Annex G */
    ...
    if (sps_svc_ext->svc_vui_parameters_present_flag) {
      read_debug_svc_vui_parameters_extension(sps_svc_ext,
                                              b); /* specified in Annex G */
    }
    break;
  default:
...
void read_debug_seq_parameter_set_svc_extension(sps_subset_t *sps_subset,
  ...
  sps_svc_ext->inter_layer_deblocking_filter_control_present_flag =
      bs_read_u1(b);
  printf(
      "sps_svc_ext->inter_layer_deblocking_filter_control_present_flag: %d \n",
      sps_svc_ext->inter_layer_deblocking_filter_control_present_flag);
  printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
  sps_svc_ext->extended_spatial_scalability_idc = bs_read_u(b, 2);
...
int read_debug_seq_parameter_set_extension_rbsp(bs_t* b, sps_ext_t* sps_ext) {
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
seq_parameter_set_id = bs_read_ue(b); printf("seq_parameter_set_id: %d \n",
seq_parameter_set_id); printf("%ld.%d: ", (long int)(b->p - b->start),
b->bits_left); aux_format_idc = bs_read_ue(b); printf("aux_format_idc: %d \n",
aux_format_idc); if( aux_format_idc != 0 ) { printf("%ld.%d: ", (long int)(b->p
- b->start), b->bits_left); bit_depth_aux_minus8 = bs_read_ue(b);
printf("bit_depth_aux_minus8: %d \n", bit_depth_aux_minus8); printf("%ld.%d: ",
(long int)(b->p - b->start), b->bits_left); alpha_incr_flag = bs_read_u1(b);
printf("alpha_incr_flag: %d \n", alpha_incr_flag); alpha_opaque_value =
bs_read_debug_u(v); alpha_transparent_value = bs_read_debug_u(v);
    }
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
additional_extension_flag = bs_read_u1(b); printf("additional_extension_flag: %d
\n", additional_extension_flag); read_debug_rbsp_trailing_bits();
}
*/
...
//7.3.2.9.2 Slice data partition B RBSP syntax
slice_data_partition_b_layer_rbsp( ) {
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left); slice_id =
bs_read_ue(b); printf("slice_id: %d \n", slice_id);     // only category 3 if(
redundant_pic_cnt_present_flag ) printf("%ld.%d: ", (long int)(b->p - b->start),
b->bits_left); redundant_pic_cnt = bs_read_ue(b); printf("redundant_pic_cnt: %d
\n", redundant_pic_cnt); read_debug_slice_data( );               // only
category 3 read_debug_rbsp_slice_trailing_bits( ); // only category 3
}

//7.3.2.9.3 Slice data partition C RBSP syntax
slice_data_partition_c_layer_rbsp( ) {
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left); slice_id =
bs_read_ue(b); printf("slice_id: %d \n", slice_id);     // only category 4 if(
redundant_pic_cnt_present_flag ) printf("%ld.%d: ", (long int)(b->p - b->start),
b->bits_left); redundant_pic_cnt = bs_read_ue(b); printf("redundant_pic_cnt: %d
\n", redundant_pic_cnt); read_debug_slice_data( );               // only
category 4 rbsp_slice_trailing_bits( ); // only category 4
}
*/
...
void read_debug_slice_header(h264_stream_t *h, bs_t *b) {
  ...
  if (sps->pic_order_cnt_type == 0) {
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
    sh->pic_order_cnt_lsb =
        bs_read_u(b, sps->log2_max_pic_order_cnt_lsb_minus4 + 4);
  ...
  if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_P) ||
    ...
    if (sh->num_ref_idx_active_override_flag) {
      ...
      printf(
          "sh->num_ref_idx_l0_active_minus1: %d \n",
          sh->num_ref_idx_l0_active_minus1); // FIXME does this modify the pps?
      if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_B)) {
        printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
        sh->num_ref_idx_l1_active_minus1 = bs_read_ue(b);
...
void read_debug_pred_weight_table(h264_stream_t *h, bs_t *b) {
  ...
  if (sps->chroma_format_idc !=
      0) // FIXME ChromaArrayType may differ from chroma_format_idc
  {
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
    sh->pwt.chroma_log2_weight_denom = bs_read_ue(b);
...
void read_debug_slice_header_in_scalable_extension(h264_stream_t *h, bs_t *b) {
  ...
  // memcpy(sps_subset, h->sps_subset_table[pps->seq_parameter_set_id],
  // sizeof(sps_subset_t));
  memcpy(sps_subset->sps, h->sps_subset_table[pps->seq_parameter_set_id]->sps,
         sizeof(sps_t));
  memcpy(sps_subset->sps_svc_ext,
         h->sps_subset_table[pps->seq_parameter_set_id]->sps_svc_ext,
         sizeof(sps_svc_ext_t));
  ...
  if (sps_subset->sps->pic_order_cnt_type == 0) {
    printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
    sh->pic_order_cnt_lsb =
        bs_read_u(b, sps_subset->sps->log2_max_pic_order_cnt_lsb_minus4 + 4);
    printf("sh->pic_order_cnt_lsb: %d \n", sh->pic_order_cnt_lsb); // was u(v)
    if (pps->pic_order_present_flag && !sh->field_pic_flag) {
      printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
      sh->delta_pic_order_cnt_bottom = bs_read_se(b);
    ...
    if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_EP) ||
      ...
      if (sh->num_ref_idx_active_override_flag) {
        ...
        printf("sh->num_ref_idx_l0_active_minus1: %d \n",
               sh->num_ref_idx_l0_active_minus1); // FIXME does this modify the
                                                  // pps?
        if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_EB)) {
          printf("%ld.%d: ", (long int)(b->p - b->start), b->bits_left);
          sh->num_ref_idx_l1_active_minus1 = bs_read_ue(b);
...
void read_debug_dec_ref_base_pic_marking(nal_t *nal, bs_t *b) {
  ...
  if (nal->prefix_nal_svc->adaptive_ref_base_pic_marking_mode_flag) {
    do {
      ...
      nal->prefix_nal_svc->memory_management_base_control_operation =
          bs_read_ue(b);
      printf("nal->prefix_nal_svc->memory_management_base_control_operation: "
             "%d \n",
             nal->prefix_nal_svc->memory_management_base_control_operation);
