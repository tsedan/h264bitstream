...
void structure(nal_unit_header_svc_extension)(nal_svc_ext_t *nal_svc_ext,
                                              bs_t *b) {
...
void structure(prefix_nal_unit_svc)(nal_t *nal, bs_t *b) {
  if (nal->nal_ref_idc != 0) {
    ...
    while (more_rbsp_data(b)) {
      value(nal->prefix_nal_svc->additional_prefix_nal_unit_extension_data_flag,
            u1);
    }
  }
}
...
void structure(subset_seq_parameter_set_rbsp)(sps_subset_t *sps_subset,
                                              bs_t *b) {
  ...
  case 83:
  case 86:
    structure(seq_parameter_set_svc_extension)(sps_subset,
    ...
    if (sps_svc_ext->svc_vui_parameters_present_flag) {
      structure(svc_vui_parameters_extension)(sps_svc_ext,
                                              b); /* specified in Annex G */
    }
    break;
  default:
...
void structure(seq_parameter_set_svc_extension)(sps_subset_t *sps_subset,
                                                bs_t *b) {
  sps_svc_ext_t *sps_svc_ext = sps_subset->sps_svc_ext;
...
void structure(svc_vui_parameters_extension)(sps_svc_ext_t *sps_svc_ext,
                                             bs_t *b) {
...
void structure(slice_header)(h264_stream_t *h, bs_t *b) {
  ...
  if (sps->pic_order_cnt_type == 0) {
    value(sh->pic_order_cnt_lsb,
          u(sps->log2_max_pic_order_cnt_lsb_minus4 + 4)); // was u(v)
  ...
  if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_P) ||
    ...
    if (sh->num_ref_idx_active_override_flag) {
      value(sh->num_ref_idx_l0_active_minus1,
            ue); // FIXME does this modify the pps?
      if (is_slice_type(sh->slice_type, SH_SLICE_TYPE_B)) {
...
void structure(slice_header_in_scalable_extension)(h264_stream_t *h, bs_t *b) {
  ...
  value(sh->frame_num,
        u(sps_subset->sps->log2_max_frame_num_minus4 + 4)); // was u(v)
  ...
  if (sps_subset->sps->pic_order_cnt_type == 0) {
    value(
        ...
        u(sps_subset->sps->log2_max_pic_order_cnt_lsb_minus4 + 4)); // was u(v)
    if (pps->pic_order_present_flag && !sh->field_pic_flag) {
      value(sh->delta_pic_order_cnt_bottom, se);
    }
  }
