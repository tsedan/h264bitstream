...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    while (find_nal_unit(p, sz, &nal_start, &nal_end) > 0) {
      ...
      case NAL_UNIT_TYPE_CODED_SLICE_AUX:
        ...
        if (pps_buf[h->sh->pic_parameter_set_id] != NULL) {
          fwrite(pps_buf[h->sh->pic_parameter_set_id], 1,
                 pps_buf_size[h->sh->pic_parameter_set_id], outfile_base);
      ...
      case NAL_UNIT_TYPE_SUBSET_SPS:
        ...
        memset(fname_buf, 0, 1024);
        sprintf(fname_buf, "%s.l_%d", argv[1],
                h->sps_subset->sps->seq_parameter_set_id);
        outfile_layers[h->sps_subset->sps->seq_parameter_set_id] =
            fopen(fname_buf, "wb");
      ...
      case NAL_UNIT_TYPE_CODED_SLICE_SVC_EXTENSION:
        ...
        if (pps_buf[h->sh->pic_parameter_set_id] != NULL) {
          fwrite(pps_buf[h->sh->pic_parameter_set_id], 1,
                 pps_buf_size[h->sh->pic_parameter_set_id],
                 outfile_layers[h->pps_table[h->sh->pic_parameter_set_id]
    ...
    if (p == buf) {
      fprintf(stderr,
              "!! Did not find any NALs between offset %lld (0x%04llX), size "
              "%lld (0x%04llX), discarding \n",
              (long long int)off, (long long int)off, (long long int)off + sz,
