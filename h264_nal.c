...
h264_stream_t *h264_new() {
  ...
  h->nal->prefix_nal_svc =
      (prefix_nal_svc_t *)calloc(1, sizeof(prefix_nal_svc_t));
  ...
  for (int i = 0; i < 64; i++) {
    ...
    h->sps_subset_table[i]->sps_svc_ext =
        (sps_svc_ext_t *)calloc(1, sizeof(sps_svc_ext_t));
  }
  for (int i = 0; i < 256; i++) {
  ...
  h->sh_svc_ext =
      (slice_header_svc_ext_t *)calloc(1, sizeof(slice_header_svc_ext_t));
...
int find_nal_unit(uint8_t *buf, int size, int *nal_start, int *nal_end) {
  ...
  *nal_start = i;
  ...
  while ( //( next_bits( 24 ) != 0x000000 && next_bits( 24 ) != 0x000001 )
      ...
      (buf[i] != 0 || buf[i + 1] != 0 || buf[i + 2] != 0x01)) {
    i++;
    // FIXME the next line fails when reading a nal that ends exactly at the end
    // of the data
    if (i + 3 >= size) {
      *nal_end = size;
      return -1;
...
/**
   ...
   The size of nal_buf must be 3/2 * the size of the rbsp_buf (rounded up) to
   guarantee the output will fit. If that is not true, output may be truncated
   and an error will be returned. If that is true, there is no possible error
   ...
   @param[in,out] nal_size  as input, pointer to the maximum size of the nal
   data; as output, filled in with the actual size of the nal data
   @return  actual size of nal data, or -1 on error
 */
// 7.3.1 NAL unit syntax
// 7.4.1.1 Encapsulation of an SODB within an RBSP
int rbsp_to_nal(const uint8_t *rbsp_buf, const int *rbsp_size, uint8_t *nal_buf,
                int *nal_size) {
  int i;
  int j = 1;
  int count = 0;
...
/**
   ...
   The size of rbsp_buf must be the same as size of the nal_buf to guarantee the
   output will fit. If that is not true, output may be truncated and an error
   will be returned. Additionally, certain byte sequences in the input nal_buf
   are not allowed in the spec and also cause the conversion to fail and an
   error to be returned.
   @param[in] nal_buf   the nal data
   @param[in,out] nal_size  as input, pointer to the size of the nal data; as
   output, filled in with the actual size of the nal data
   @param[in,out] rbsp_buf   allocated memory in which to put the rbsp data
   @param[in,out] rbsp_size  as input, pointer to the maximum size of the rbsp
   data; as output, filled in with the actual size of rbsp data
   @return  actual size of rbsp data, or -1 on error
 */
// 7.3.1 NAL unit syntax
// 7.4.1.1 Encapsulation of an SODB within an RBSP
int nal_to_rbsp(const uint8_t *nal_buf, int *nal_size, uint8_t *rbsp_buf,
                int *rbsp_size) {
  int i;
  int j = 0;
  int count = 0;
  ...
  for (i = 0; i < *nal_size; i++) {
    ...
    if ((count == 2) && (nal_buf[i] == 0x03)) {
      ...
      // if cabac_zero_word is used, the final byte of this NAL unit(0x03) is
      // discarded, and the last two bytes of RBSP must be 0x0000
      if (i == *nal_size - 1) {
        break;
      }
