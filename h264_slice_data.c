...
void read_macroblock_layer(h264_stream_t *h, bs_t *b) {
  ...
  if (mb->mb_type == I_PCM) {
    ...
    if (mb->mb_type != I_NxN && MbPartPredMode(mb->mb_type, 0) != Intra_16x16 &&
    ...
    if (MbPartPredMode(mb->mb_type, 0) != Intra_16x16) {
      ...
      if (CodedBlockPatternLuma > 0 && h->pps->transform_8x8_mode_flag &&
          mb->mb_type != I_NxN && noSubMbPartSizeLessThan8x8Flag &&
    ...
    for (int iCbCr = 0; iCbCr < 2; iCbCr++) {
      for (int i8x8 = 0; i8x8 < NumC8x8; i8x8++) {
        for (int i4x4 = 0; i4x4 < 4; i4x4++) {
          ...
          {
            read_residual_block(b, mb->ChromaACLevel[iCbCr][i8x8 * 4 + i4x4],
                                15);
          } else {
            for (int i = 0; i < 15; i++) {
    ...
    for (int iCbCr = 0; iCbCr < 2; iCbCr++) {
      for (int i8x8 = 0; i8x8 < NumC8x8; i8x8++) {
        for (int i4x4 = 0; i4x4 < 4; i4x4++) {
          if (CodedBlockPatternChroma & 2) // chroma AC residual present
          {
            write_residual_block(b, mb->ChromaACLevel[iCbCr][i8x8 * 4 + i4x4],
                                 15);
          } else {
            for (int i = 0; i < 15; i++) {
              mb->ChromaACLevel[iCbCr][i8x8 * 4 + i4x4][i] = 0;
  ...
  if (mb->mb_type == I_PCM) {
    while (!bs_byte_aligned(b)) {
      printf("%d.%d: ", b->p - b->start,
             b->bits_left); // ERROR: value( pcm_alignment_zero_bit, f(1) );
  ...
  for (int i8x8 = 0; i8x8 < 4; i8x8++) // each luma 8x8 block
  {
    if (!mb->transform_size_8x8_flag || !h->pps->entropy_coding_mode_flag) {
      for (int i4x4 = 0; i4x4 < 4; i4x4++) // each 4x4 sub-block of block
      {
        if (CodedBlockPatternLuma & (1 << i8x8)) {
          if (MbPartPredMode(mb->mb_type, 0) == Intra_16x16) {
            read_debug_residual_block(b, mb->Intra16x16ACLevel[i8x8 * 4 + i4x4],
                                      15);
          } else {
    ...
    for (int iCbCr = 0; iCbCr < 2; iCbCr++) {
      for (int i8x8 = 0; i8x8 < NumC8x8; i8x8++) {
        for (int i4x4 = 0; i4x4 < 4; i4x4++) {
          if (CodedBlockPatternChroma & 2) // chroma AC residual present
          {
            read_debug_residual_block(
                b, mb->ChromaACLevel[iCbCr][i8x8 * 4 + i4x4], 15);
          } else {
            for (int i = 0; i < 15; i++) {
              mb->ChromaACLevel[iCbCr][i8x8 * 4 + i4x4][i] = 0;
            }
...
// 7.3.5.3.1 Residual block CAVLC syntax
void read_debug_residual_block_cavlc(bs_t *b, int *coeffLevel,
                                     int maxNumCoeff) {
  int level[256];
  int run[256];
  for (int i = 0; i < maxNumCoeff; i++) {
    ...
    for (int i = 0; i < TotalCoeff(coeff_token); i++) {
      if (i < TrailingOnes(coeff_token)) {
        ...
        if (suffixLength > 0 || level_prefix >= 14) {
          int level_suffix;
          printf("%d.%d: ", b->p - b->start,
                 b->bits_left); // ERROR: value( level_suffix, u );
                                // printf("level_suffix: %d \n", level_suffix);
                                // // FIXME
          levelCode += level_suffix;
        }
        if (level_prefix >= 15 && suffixLength == 0) {
...
#ifdef HAVE_CABAC
// 7.3.5.3.2 Residual block CABAC syntax
void read_debug_residual_block_cabac(bs_t *b, int *coeffLevel,
                                     int maxNumCoeff) {
  if (maxNumCoeff == 64) {
    coded_block_flag = 1;
  } else {
